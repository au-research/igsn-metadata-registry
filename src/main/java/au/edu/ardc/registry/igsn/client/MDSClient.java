...
public class MDSClient {
	...
	public int mintIGSN(String registrationMetadata, String identifier, String landingPage, boolean testMode) throws Exception {
		int response_code;
		try {
	...
	private int createIdentifier(String identifier, String landing_page, boolean testMode) throws Exception {
		...
		try {
			ClientResponse response = this.web_client.post()
			.uri(service_url)
			.contentType(MediaType.TEXT_PLAIN)
			.body(BodyInserters.fromPublisher(Mono.just(mint_content), String.class))
			.exchange().block();
	...
	private int addMetadata(String registrationMetadata) throws Exception {
		...
		try {
			...
			.body(BodyInserters.fromPublisher(Mono.just(registrationMetadata), String.class))
			.exchange().block();
	...
	public String getIGSNMetadata(String identifier) throws Exception {
		...
		try {
			ClientResponse response = this.web_client.get()
			.uri(service_url)
			.exchange().block();
